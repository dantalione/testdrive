...
function wireEvents() {
  ...
  document.getElementById('loadBtn').addEventListener('click', async () => {
    try {
      ...
      let marketRaw = await parseOptionalObjectFile(document.getElementById('marketFile'));
      let notasiRaw = await parseOptionalObjectFile(document.getElementById('notasiFile'));
      let freeFloatRaw = await parseOptionalObjectFile(document.getElementById('freeFloatFile'));
      const olderRows = dedupeRows(olderRaw);
      const newerRows = dedupeRows(newerRaw);
      ...
      if (!freeFloatRaw) {
        ...
        freeFloatSource = autoFreeFloat.path;
      } else {
        freeFloatSource = 'manual_input';
      }
